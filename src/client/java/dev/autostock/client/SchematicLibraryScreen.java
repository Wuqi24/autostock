...
final class SchematicLibraryScreen extends CompatGuiBase {
    ...
    @Override public void initGui(){super.initGui();int rows=Math.max(1,(height-108)/24);var schematics=Schematics.loaded();page=Math.min(page,Math.max(0,(schematics.size()-1)/rows));
        ...
        if(schematics.isEmpty()&&AutoStockClient.activeDraft!=null&&!AutoStockClient.activeDraft.materials().isEmpty())addButton(new fi.dy.masa.malilib.gui.button.ButtonGeneric(12,100,width-24,20,AutoStockClient.activeDraft.schematicName(),"备货需求快照 · 非已加载原理图文件"),(b,m)->client.setScreen(new DemandScreen(AutoStockClient.activeDraft,this)));
        for(int i=0;i<rows&&page*rows+i<schematics.size();i++){var schematic=schematics.get(page*rows+i);var file=SchematicCompat.file(schematic);addButton(new fi.dy.masa.malilib.gui.button.ButtonGeneric(12,72+i*24,width-24,20,schematic.getMetadata().getName(),schematic.getSubRegionCount()+" 子区域 · "+(file==null?"内存原理图":file.getFileName().toString())),(b,m)->client.setScreen(new SchematicDetailScreen(schematic,this)));}
    }
    @Override protected void drawScreenBackgroundCompat(DrawContext c,int x,int y){c.fill(0,0,width,height,0x80000000);}
    @Override protected void drawContentsCompat(DrawContext c,int x,int y,float delta){c.drawText(textRenderer,"原理图文件与已加载列表",12,16,0xFFE8E8EA,false);if(Schematics.loaded().isEmpty())c.drawText(textRenderer,"无已加载文件；可浏览文件，或查看下方需求快照",12,85,0xFF9A9AA2,false);}
}
