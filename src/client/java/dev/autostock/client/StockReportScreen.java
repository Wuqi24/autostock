...
final class StockReportScreen extends CompatGuiBase {
    ...
    //?} else {
    /*@Override public boolean onMouseClicked(net.minecraft.client.gui.Click click,boolean doubled){return handleMouseClicked(click.x(),click.y(),click.button())||super.onMouseClicked(click,doubled);}
    *///?}
    static String itemName(String id) { return Registries.ITEM.get(Identifier.of(id)).getName().getString(); }
    ...
    private void line(DrawContext c, String text, int x, int y, int available, int color) {
        c.drawTextWithShadow(textRenderer, textRenderer.trimToWidth(text, Math.max(1, available)), x, y, color);
    }
}
