...
final class HudPanels {
    ...
    static void render(DrawContext context, ClientDraft draft, boolean selection) {
    ...
    static void text(DrawContext context, String value, int x, int y, int width, int color) {
        var font = MinecraftClient.getInstance().textRenderer;
        String fitted = HudLayout.ellipsize(value, Math.max(0, width), font::getWidth);
        context.drawText(font, fitted, x, y, color, false);
    }
    ...
    static String itemName(String id) {
        ...
        return key == null ? id : Registries.ITEM.get(key).getName().getString();
    }

    static void renderPanel(DrawContext context, ClientDraft draft, Rect rect, int panel, Style style) {
        ...
        try {
            ...
            *///?} else {
            ...
            try {
                ...
                *///?} else {
                context.getMatrices().translate(rect.x() + 4, rect.y() + 1);
                context.getMatrices().scale(CONTENT_SCALE, CONTENT_SCALE);
                //?}
    ...
    private static void drawChrome(DrawContext context, Rect rect, int panel, TaskStatus task, Style style) {
        int x = rect.x();
        int y = rect.y();
        ...
        if (panel == TASK && task != null) {
            ...
            int stateWidth = Math.round(font.getWidth(state) * CONTENT_SCALE);
    ...
    private static void renderRegion(DrawContext context, ClientDraft draft, int width, Style style) {
        ...
        for (int index = 0; index < ROLES.length; index++) {
            ...
            String fitted = HudLayout.ellipsize(value, Math.max(1, cellWidth - 2), font::getWidth);
            int textX = left + Math.max(1, (cellWidth - font.getWidth(fitted)) / 2);
            context.drawText(font, fitted, textX, 22, color, false);
        }
        centered(context, regionStatus(), 39, width, style.accent());
        centered(context, hint(), 58, width, dim(style.text()));
    }

    private static void renderTask(DrawContext context, TaskStatus task, int width, Style style) {
        var font = MinecraftClient.getInstance().textRenderer;
    ...
    private static void pair(DrawContext context, String label, String value, int y, int width, Style style,
    ...
    private static void centered(DrawContext context, String value, int y, int width, int color) {
        TextRenderer font = MinecraftClient.getInstance().textRenderer;
        String fitted = HudLayout.ellipsize(value, Math.max(1, width - 12), font::getWidth);
        context.drawText(font, fitted, Math.max(6, (width - font.getWidth(fitted)) / 2), y, color, false);
    }

    private static void rightText(DrawContext context, String value, int left, int y, int right, int color) {
        TextRenderer font = MinecraftClient.getInstance().textRenderer;
        String fitted = HudLayout.ellipsize(value, Math.max(1, right - left), font::getWidth);
        context.drawText(font, fitted, Math.max(left, right - font.getWidth(fitted)), y, color, false);
    }
