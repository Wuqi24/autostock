...
final class InventoryView {
    ...
    static void tick(MinecraftClient client){
        boolean visible=client.currentScreen instanceof BotInventoryScreen||client.currentScreen instanceof PreviewScreen p&&p.watchesBot()||client.currentScreen instanceof PreviewDialog dialog&&dialog.getParent() instanceof PreviewScreen parent&&parent.watchesBot();
        if(visible&&session==null&&client.getNetworkHandler()!=null&&ClientPlayNetworking.canSend(InventoryWatch.ID)){
        ...
        }else if(!visible&&session!=null){if(client.getNetworkHandler()!=null&&ClientPlayNetworking.canSend(InventoryWatch.ID))ClientPlayNetworking.send(new InventoryWatch(session,false));clear();}
    }
    ...
    static String status(){
        ...
        if(world!=null&&!world.getRegistryKey().getValue().toString().equals(snapshot.dimension()))return "假人不在当前维度";
        return "在线 · 实时同步";
    }
}
