...
final class PreviewDialog extends CompatGuiBase {
    ...
    @Override public void initGui() {
        ...
        contentHeight = 0;
        for (String line : lines.get()) contentHeight += textRenderer.wrapLines(Text.literal(line), panelWidth - 24).size() * 13;
    ...
    @Override protected void drawScreenBackgroundCompat(DrawContext c, int x, int y) {
    ...
    @Override protected void drawContentsCompat(DrawContext c, int x, int y, float delta) {
        c.drawText(textRenderer, textRenderer.trimToWidth(heading, panelWidth - 24), left + 12, top + 12, 0xFFFFFFFF, true);
        c.enableScissor(left + 8, top + 32, left + panelWidth - 8, top + panelHeight - 36);
        int row = top + 34 - offset;
        for (String line : lines.get()) for (var wrapped : textRenderer.wrapLines(Text.literal(line), panelWidth - 24)) {
            c.drawText(textRenderer, wrapped, left + 12, row, 0xFFDDDDDD, false);
            row += 13;
        }
