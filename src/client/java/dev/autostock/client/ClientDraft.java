...
public final class ClientDraft {
    ...
    public void tick() {
        ...
        if (dimension != null && (world == null || !dimension.equals(world.getRegistryKey().getValue().toString()))) {
            cancelScan();
            discard();
    ...
    private void requireWorld() {
        ...
        if (dimension == null) dimension = world.getRegistryKey().getValue().toString();
    }
    ...
    private void cancelScan() {
        if (scanning && pendingRequest != null && MinecraftClient.getInstance().getNetworkHandler() != null
    ...
    private void notifyFake(String message) {
        ...
        if (player != null) {var settings=ClientSettings.get();player.sendMessage(Text.literal(settings.notificationPrefix + message), false);if(settings.notificationSound)player.playSound(net.minecraft.sound.SoundEvents.ENTITY_EXPERIENCE_ORB_PICKUP,.3F,1F);}
    }
