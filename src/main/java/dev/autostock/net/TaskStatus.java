...
public record TaskStatus(UUID taskId,String state,String detail,long delivered,long inTransit,long required,
                         BlockPos target,List<BlockPos> path,boolean bindingsVerified,String material,long materialRemaining,long etaSeconds) implements CustomPayload {
    ...
    public static final Id<TaskStatus> ID=new Id<>(Identifier.of("autostock","task_status_v4"));
    public static final PacketCodec<RegistryByteBuf,TaskStatus> CODEC=new PacketCodec<>() {
        public TaskStatus decode(RegistryByteBuf buf) {
            var task=buf.readUuid();var state=buf.readString(32);var detail=buf.readString(512);
            ...
            return new TaskStatus(task,state,detail,w,t,d,target,path,buf.readBoolean(),buf.readString(128),buf.readLong(),buf.readLong());
        }
        public void encode(RegistryByteBuf buf,TaskStatus value) {
            buf.writeUuid(value.taskId());buf.writeString(value.state(),32);buf.writeString(value.detail(),512);
            ...
            buf.writeVarInt(value.path().size());value.path().forEach(buf::writeBlockPos);buf.writeBoolean(value.bindingsVerified());buf.writeString(value.material(),128);buf.writeLong(value.materialRemaining());buf.writeLong(value.etaSeconds());
        }
    };
