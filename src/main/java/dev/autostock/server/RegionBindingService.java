...
public final class RegionBindingService {
    private record Job(ServerPlayerEntity player, RegionBinding request, WarehouseIndex index, int started) { }

    private final Map<UUID, Job> jobs = new HashMap<>();
    private final com.google.gson.Gson gson = new com.google.gson.Gson();

    public void register() {
        PayloadTypeRegistry.playC2S().register(RegionScan.ID, RegionScan.CODEC);
        PayloadTypeRegistry.playS2C().register(RegionBound.ID, RegionBound.CODEC);
        ServerPlayNetworking.registerGlobalReceiver(RegionScan.ID, (payload, context) -> {
            ...
            try {
                ...
                if (request == null || request.request() == null
                        || !world.getRegistryKey().getValue().toString().equals(request.dimension())
                ...
                for (var entry : request.regions().entrySet()) {
                    ...
                    for (var box : entry.getValue()) {
                        ...
                        if (!world.getWorldBorder().contains(new BlockPos(box.minX(), box.minY(), box.minZ()))
                                || !world.getWorldBorder().contains(new BlockPos(box.maxX(), box.maxY(), box.maxZ()))
                                || volume > PlanDraft.MAX_REGION_VOLUME
                                || box.minY() < world.getBottomY()
                                || box.maxY() >= world.getBottomY() + world.getHeight()
                ...
                jobs.put(player.getUuid(), new Job(player, request,
                        new WarehouseIndex(world, request.regions(), Map.of()), context.server().getTicks()));
        ...
        ServerTickEvents.END_SERVER_TICK.register(server -> {
            for (var job : List.copyOf(jobs.values())) {
                try {
                    if (server.getPlayerManager().getPlayer(job.player().getUuid()) != job.player()) {
                        jobs.remove(job.player().getUuid());
                        continue;
                    }
                    if (server.getTicks() - job.started() > 1600) throw new IllegalArgumentException("区域扫描超时");
                    ...
                    for (var container : job.index().containers) {
                        ...
                        if (container.role() == RegionRole.OUTPUT) {
                            ...
                            for (int i = 0; i < container.inventory().size(); i++) {
                                if (container.inventory().getStack(i).isEmpty()) slots++;
                            }
                        }
                        if (container.role() == RegionRole.EMPTY_BOX) {
                            for (int i = 0; i < container.inventory().size(); i++) {
                                var stack = container.inventory().getStack(i);
    ...
    private void reply(ServerPlayerEntity player, RegionBinding request, Map<RegionRole, Integer> counts,
