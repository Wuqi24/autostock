...
final class QuickShulkerSession implements BoxSession {
    ...
    private final ShulkerBoxScreenHandler handler;

    static QuickShulkerSession open(ServerPlayerEntity player, int inventorySlot) {
        ...
        var stack = player.getInventory().getStack(inventorySlot);
        if (!InventoryItems.isBox(stack) || stack.getCount() != 1) throw new IllegalArgumentException("该槽位必须恰好有一个潜影盒");
        if (stack.contains(DataComponentTypes.CONTAINER_LOOT)) throw new IllegalArgumentException("请先手动解析盒内战利品");
        ...
        if (!(player.currentScreenHandler instanceof ShulkerBoxScreenHandler opened)
    ...
    private QuickShulkerSession(ServerPlayerEntity player, int slot, ItemStack stack, ShulkerBoxScreenHandler handler) {
    ...
    void verify() {
        ...
        if (player.currentScreenHandler != handler || player.getInventory().getStack(inventorySlot) != original
    ...
    @Override public void closeVerified() {
        verify();
        if (!handler.getCursorStack().isEmpty()) throw new IllegalArgumentException("光标仍持有物品，保留会话等待处理");
        List<ItemStack> expected = java.util.stream.IntStream.range(0, 27)
                .mapToObj(i -> handler.getSlot(i).getStack().copy()).toList();
        player.closeHandledScreen();
        if (player.currentScreenHandler != player.playerScreenHandler || player.getInventory().getStack(inventorySlot) != original) {
            throw new IllegalArgumentException("关闭后会话或盒身份异常，请人工检查；不会补发物品");
        }
        var actual = DefaultedList.ofSize(27, ItemStack.EMPTY);
        original.getOrDefault(DataComponentTypes.CONTAINER, ContainerComponent.DEFAULT).copyTo(actual);
        for (int i = 0; i < 27; i++) if (!ItemStack.areEqual(expected.get(i), actual.get(i))) {
    ...
    void transfer(int sourceIndex, int destinationIndex, int count) {
        ...
        var item = source.getStack().copy();
        if (!item.isEmpty()) BuildingMaterials.requireSupported(item.getItem());
        var existing = destination.getStack().copy();
        if (item.isEmpty() || count > item.getCount() || !source.canTakeItems(player) || !destination.canInsert(item)
                || (!existing.isEmpty() && !ItemStack.areItemsAndComponentsEqual(item, existing))
                || count > destination.getMaxItemCount(item) - existing.getCount()) {
        ...
        outerBefore.remove(DataComponentTypes.CONTAINER);
        click(sourceIndex, 0);
        if (count == item.getCount()) click(destinationIndex, 0);
        else for (int i = 0; i < count; i++) click(destinationIndex, 1);
        if (!handler.getCursorStack().isEmpty()) click(sourceIndex, 0);
        verify();
        if (!handler.getCursorStack().isEmpty()) throw new IllegalArgumentException("转移后光标未清空，保留现场等待处理");
        for (int i = 0; i < before.size(); i++) {
            ...
            if (!ItemStack.areEqual(expected, handler.getSlot(i).getStack())) {
                throw new IllegalArgumentException("转移后槽位核对不符，保留现场；不会自动补发或回滚");
            }
        }
        var outerAfter = original.copy();
        outerAfter.remove(DataComponentTypes.CONTAINER);
        if (!ItemStack.areEqual(outerBefore, outerAfter)) throw new IllegalArgumentException("外层盒属性异常，停止并保留现场");
    }

    private boolean isOuterSlot(int index) {
        var slot = handler.getSlot(index);
        return slot.inventory == player.getInventory() && slot.getIndex() == inventorySlot;
    }
    ...
    @Override public void move(int playerSlot, int boxSlot, int count, boolean intoBox) {
        ...
        for (int i = 27; i < handler.slots.size(); i++) {
            ...
            if (slot.inventory == player.getInventory() && slot.getIndex() == playerSlot) {
    ...
    private void actionReady() {
        ...
        if (!handler.getCursorStack().isEmpty()) throw new IllegalArgumentException("光标有物品，先处理现场");
    }
    ...
    private void click(int index, int button) {
        ...
        handler.onSlotClick(index, button, SlotActionType.PICKUP, player);
    }
}
//?}
