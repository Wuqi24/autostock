...
final class TaskRuntime {
    ...
    TaskRuntime(EntityPlayerMPFake player,UUID task,PlanDraft draft,TaskJournal journal,SourceBoxFlow.Origin origin) throws IOException {
        ...
        if (!MinecraftCompat.world(player).getRegistryKey().getValue().toString().equals(draft.dimension()))
        ...
        if (!player.currentScreenHandler.getCursorStack().isEmpty() || player.currentScreenHandler != player.playerScreenHandler)
    ...
    List<SafePath.Node> path() { return state!=TaskJournal.State.RUNNING || MinecraftCompat.server(player).getPlayerManager().getPlayer(player.getUuid())!=player || walker == null ? List.of() : List.copyOf(walker.remainingPath()); }
    ...
    void tick() {
        boolean online=MinecraftCompat.server(player).getPlayerManager().getPlayer(player.getUuid())==player;
    ...
    private ItemStack stack(int slot) { return player.getInventory().getStack(slot); }
    private void preflight() throws IOException {
        ...
        java.util.function.Consumer<ItemStack> add=item->{
            ...
            supplies.add(new dev.autostock.core.StockPlanner.Supply(id(item),variant,Math.min(64,item.getMaxCount()),item.getCount(),true));
        };
        for(var c:warehouse.containers)if(c.role()==RegionRole.MATERIAL)for(int i=0;i<c.inventory().size();i++) {
            var item=c.inventory().getStack(i);if(InventoryItems.isBox(item))InventoryItems.contents(item).forEach(add);else add.accept(item);
        }
    ...
    private static String id(ItemStack stack) { return Registries.ITEM.getId(stack.getItem()).toString(); }
    ...
    private List<ItemStack> boxSlots(ItemStack box) {
        var values=new ArrayList<ItemStack>(); box.getOrDefault(DataComponentTypes.CONTAINER,ContainerComponent.DEFAULT).stream().forEach(values::add);
    ...
    private int fit(ItemStack material) {
        ...
        for(int i=0;i<27;i++) if(!slots.get(i).isEmpty() && ItemStack.areItemsAndComponentsEqual(slots.get(i),material)
                && slots.get(i).getCount()<Math.min(64,material.getMaxCount()))return i;
        for(int i=0;i<27;i++)if(slots.get(i).isEmpty())return i;
        return -1;
    }
    private int capacity(int boxSlot,ItemStack material) { return Math.min(64,material.getMaxCount())-boxSlots(stack(0)).get(boxSlot).getCount(); }
    private void moveBag(int from,int to) {
        if(!stack(to).isEmpty())throw new IllegalArgumentException("背包目标槽位已占用");
        player.getInventory().setStack(to,stack(from)); player.getInventory().setStack(from,ItemStack.EMPTY);
        player.getInventory().markDirty(); player.playerScreenHandler.sendContentUpdates();
    }
    private void advance() throws IOException {
        ...
        var selected=sources.stream().filter(c->c.position().equals(materialTarget)).findFirst()
                .orElseGet(()->sources.stream().min(Comparator
                        .comparingInt((WarehouseIndex.Container c)->chebyshev(player.getBlockPos(),c.position()))
    ...
    private boolean hasNeeded(WarehouseIndex.Container c,Map<String,Long> transit) {
        ...
        for(int slot=0;slot<c.inventory().size();slot++) {
            var stack=c.inventory().getStack(slot);
    ...
    private void takeMaterial(WarehouseIndex.Container c) {
        ...
        for(int slot=0;slot<c.inventory().size();slot++) {
            var material=c.inventory().getStack(slot);
    ...
    private void packLoose(Map<String,Long> packed) {
        detail="装盒 · "+stack(10).getName().getString();
    ...
    private void acquireWorkBox() {
        ...
        var candidates=warehouse.containers.stream().filter(c->c.role()==RegionRole.EMPTY_BOX).filter(c->{
            c.verify(MinecraftCompat.world(player));for(int i=0;i<c.inventory().size();i++)if(emptyBox(c.inventory().getStack(i)))return true;return false;
        }).toList();
        if(candidates.isEmpty())throw new IllegalArgumentException("空盒不足，请补充空盒区");
        travel(candidates,c->{for(int i=0;i<c.inventory().size();i++)if(emptyBox(c.inventory().getStack(i))) { transfer(c,i,0,1,true);return; }
    ...
    private void deliver(int slot) {
        ...
        var destination=warehouse.containers.stream().filter(c->c.role()==RegionRole.OUTPUT).filter(c->{
            c.verify(MinecraftCompat.world(player));for(int i=0;i<c.inventory().size();i++)if(c.inventory().getStack(i).isEmpty())return true;return false;
        }).findFirst().orElseThrow(()->new IllegalArgumentException("备货区已满，成品保留在假人背包"));
        travel(List.of(destination),c->{for(int i=0;i<c.inventory().size();i++)if(c.inventory().getStack(i).isEmpty()) {
    ...
    private void transfer(WarehouseIndex.Container c,int containerSlot,int bagSlot,int count,boolean intoBag) {
        ...
        if(containerSlot<0||containerSlot>=handler.slots.size()||handler.getSlot(containerSlot).inventory==player.getInventory())throw new IllegalArgumentException("容器槽位结构改变");
