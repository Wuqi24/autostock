...
final class FakeWalker {
    ...
    FakeWalker(ServerPlayerEntity player, BlockPos target, boolean interaction) {
        if (!(player instanceof EntityPlayerMPFake)) throw new IllegalArgumentException("仅支持 Carpet 假人移动");
        this.player = player; this.target = target.toImmutable(); this.interaction = interaction;
        cargo = new WalkingCargo(player);
        search = new SafePath(player,target,interaction); actions().stopAll();
    }
    FakeWalker(ServerPlayerEntity player, SafePath.Route planned) {
    ...
    void stop() { actions().stopAll();if(originalSpeed>=0)player.getAttributeInstance(MinecraftCompat.movementSpeed()).setBaseValue(originalSpeed); }
    ...
    void tick() {
        ...
        if (!MinecraftCompat.server(player).isOnThread()) throw new IllegalArgumentException("移动必须在服务器线程执行");
        ...
        var options=RuntimeOptions.get(player.getUuid());var speedAttribute=player.getAttributeInstance(MinecraftCompat.movementSpeed());if(originalSpeed<0)originalSpeed=speedAttribute.getBaseValue();speedAttribute.setBaseValue(originalSpeed*options.moveSpeed()/100.0);
        if (++ticks > 2400/Math.max(.15,options.moveSpeed()/100.0*(options.silent()?.3:1))) { fail("前往目标超过 120 秒"); return; }
        if (player.currentScreenHandler != player.playerScreenHandler || !player.currentScreenHandler.getCursorStack().isEmpty()) { fail("移动前容器或光标尚未结束"); return; }
        ...
        if (distance < .17 && Math.abs(point.y - player.getY()) < .16 && player.isOnGround()) {
        ...
        if (point.y > player.getY() + .6 && player.isOnGround())
    ...
    private boolean openDoor(BlockPos pos) {
        ...
        if (state.get(door ? DoorBlock.OPEN : TrapdoorBlock.OPEN)) return true;
        var eye = player.getEyePos();
        var shape = state.getOutlineShape(world,pos,net.minecraft.block.ShapeContext.of(player));
        if (shape.isEmpty()) return false;
        var center = shape.getBoundingBox().getCenter().add(Vec3d.of(pos));
        if (eye.squaredDistanceTo(center) > 4.5 * 4.5) return false;
        var hit = world.raycast(new RaycastContext(eye,center,RaycastContext.ShapeType.OUTLINE,RaycastContext.FluidHandling.NONE,player));
        ...
        return world.getBlockState(pos).get(door ? DoorBlock.OPEN : TrapdoorBlock.OPEN);
    }
}
