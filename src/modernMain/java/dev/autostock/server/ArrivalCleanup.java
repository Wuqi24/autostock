...
final class ArrivalCleanup {
    ...
    ArrivalCleanup(EntityPlayerMPFake player, UUID owner) throws IOException {
        ...
        bindingKeys = new OutputBindings(MinecraftCompat.server(player), owner).keys(MinecraftCompat.world(player).dimension().identifier().toString());
    ...
    static boolean emptyBox(ItemStack item) { return InventoryItems.isBox(item) && !item.has(net.minecraft.core.component.DataComponents.CONTAINER_LOOT) && !InventoryItems.contents(item).iterator().hasNext(); }
    static boolean needed(EntityPlayerMPFake player) {
        for (int i=0;i<36;i++) if (!player.getInventory().getItem(i).isEmpty() && !emptyBox(player.getInventory().getItem(i))) return true;
        return false;
    }
    ...
    void tick() {
        if (done) return;
        try {
            if (MinecraftCompat.server(player).getPlayerList().getPlayer(player.getUUID()) != player) throw new IllegalArgumentException("假人已下线");
            ...
            slot = -1;
            for (int i=0;i<36;i++) { var item=player.getInventory().getItem(i);if(!item.isEmpty()&&!emptyBox(item)){slot=i;break;} }
            if (slot < 0) { done=true;detail="上线背包已归置，空盒保留";return; }
            target = index.containers.stream().filter(c->{c.verify(MinecraftCompat.world(player));for(int i=0;i<c.inventory().getContainerSize();i++)if(c.inventory().getItem(i).isEmpty())return true;return false;}).findFirst().orElseThrow(()->new IllegalArgumentException("备货区已满，上线物品仍保留在假人背包"));
    ...
    private void deposit() throws IOException {
        target.verify(MinecraftCompat.world(player));var item=player.getInventory().getItem(slot);
        if(item.isEmpty()||emptyBox(item))return;
        int into=-1;for(int i=0;i<target.inventory().getContainerSize();i++)if(target.inventory().getItem(i).isEmpty()){into=i;break;}
        ...
        if(slot==9){var journal=new TaskJournal(MinecraftCompat.server(player).getWorldPath(LevelResource.ROOT),owner);var old=journal.load();if(old!=null&&old.source()!=null)journal.save(new TaskJournal.Entry(old.version(),old.task(),old.state(),old.dimension(),null,"来源盒已按上线归置规则存入备货区"));}
    }
