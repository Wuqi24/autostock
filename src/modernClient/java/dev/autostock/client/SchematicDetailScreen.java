...
final class SchematicDetailScreen extends CompatGuiBase {
    ...
    @Override protected void drawContentsCompat(GuiContext c,int x,int y,float delta){
        c.drawString(textRenderer,"原理图详情",12,16,0xFFE8E8EA,false);var size=schematic.getTotalSize();String[] lines={schematic.getMetadata().getName(),"尺寸 "+size.getX()+" × "+size.getY()+" × "+size.getZ()+" · "+schematic.getSubRegionCount()+" 子区",schematic.getFile()==null?"内存原理图":schematic.getFile().toString()};
        for(int i=0;i<lines.length;i++)c.drawString(textRenderer,textRenderer.plainSubstrByWidth(lines[i],width-24),12,119+i*17,0xFF9A9AA2,false);
        c.drawString(textRenderer,textRenderer.plainSubstrByWidth(message,width-24),12,height-26,0xFFFBBF24,false);
    }
}
