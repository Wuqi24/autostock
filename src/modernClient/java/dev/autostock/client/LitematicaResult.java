...
public final class LitematicaResult {
    ...
    public static void addStockButton(GuiMaterialList screen) {
        ...
        int mainWidth=client.font.width(main)+20,buttonWidth=client.font.width("假人备货")+20;
        ...
        screen.addButton(new fi.dy.masa.malilib.gui.button.ButtonGeneric(x,screen.getScreenHeight()-36,buttonWidth,20,"假人备货"),(button,mouse)->{
            try {
                ...
                draft.freezeDemand();
                var next=new DraftScreen(draft);ClientMinecraftCompat.setScreen(client,next);next.selectTab(2);
    ...
    static void capture(GuiMaterialList screen) {
        ...
        try {
            ...
            for (Object row : widget.getCurrentEntries()) {
                ...
                if (count == 0) continue;
                counts.merge(BuiltInRegistries.ITEM.getKey(entry.getStack().getItem()).toString(), count, Math::addExact);
            }
