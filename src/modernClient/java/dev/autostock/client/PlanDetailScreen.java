...
final class PlanDetailScreen extends CompatGuiBase {
    ...
    @Override protected void drawContentsCompat(GuiContext c,int x,int y,float delta){
        c.drawString(textRenderer,draft.materials().size()+" 种材料 / "+draft.totalItems()+" 件",12,136,0xFFDDE6F0,false);
        var report=draft.report();if(report!=null)c.drawString(textRenderer,"计划 "+report.plan().boxes()+" 盒 · 空盒 "+report.emptyBoxes()+" · 输出空槽 "+report.outputSlots(),12,153,0xFFAAAAAA,false);
        c.drawString(textRenderer,textRenderer.plainSubstrByWidth(message.isEmpty()?draft.status():message,width-24),12,height-28,0xFFE2C478,false);
    }
}
