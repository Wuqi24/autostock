...
final class BotInventoryScreen extends CompatGuiBase {
    ...
    @Override protected void drawScreenBackgroundCompat(GuiContext c,int x,int y){c.fill(0,0,width,height,0x80000000);c.fill(0,0,width,34,0x60303030);}
    @Override protected void drawContentsCompat(GuiContext c,int mx,int my,float delta){
        c.drawString(textRenderer,"假人详情",12,14,0xFFE8E8EA,false);
        ...
        c.drawString(textRenderer,textRenderer.plainSubstrByWidth(InventoryView.status(),textWidth),12,43,0xFF9A9AA2,false);
        ...
        if(columns){int yy=104;for(var line:textRenderer.split(net.minecraft.network.chat.Component.literal("当前动作："+behavior),textWidth)){c.drawString(textRenderer,line,12,yy,0xFFF09040,false);yy+=12;if(yy>height-45)break;}}
        else c.drawString(textRenderer,textRenderer.plainSubstrByWidth("当前动作："+behavior,width-24),12,61,0xFFF09040,false);
        ...
        if(columns){c.drawString(textRenderer,"生命值："+String.format(java.util.Locale.ROOT,"%.1f",data.health()),12,62,0xFF9A9AA2,false);c.drawString(textRenderer,textRenderer.plainSubstrByWidth("位置："+data.position().toShortString(),textWidth),12,81,0xFF9A9AA2,false);c.drawString(textRenderer,"背包",left,43,0xFFE8E8EA,false);}
        else c.drawString(textRenderer,textRenderer.plainSubstrByWidth(health,width/2-12),width/2,43,0xFF9A9AA2,false);
        int hovered=-1;
        for(int row=0;row<4;row++)for(int col=0;col<9;col++){
            ...
            var stack=data.slots().get(index);c.renderItem(stack,x+3,y+3);
            //? if <=1.21.1 {
            /*c.drawItemInSlot(textRenderer,stack,x+3,y+3);
            *///?} else {
            c.renderItemDecorations(textRenderer,stack,x+3,y+3);
            //?}
            if(mx>=x&&mx<x+21&&my>=y&&my<y+21){c.fill(x+1,y+1,x+20,y+20,0x25FFFFFF);hovered=index;}
        }
        c.drawString(textRenderer,"紫：空盒  绿：含物品盒 · 只读",left,top+100,0xFF9A9AA2,false);
        if(hovered>=0&&!data.slots().get(hovered).isEmpty())c.setTooltipForNextFrame(textRenderer,data.slots().get(hovered),mx,my);
    }
}
